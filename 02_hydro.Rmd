...
df2 <- df %>%
  ...
  group_by(stationname, jaar, parametername) %>% summarise(median = median(value, na.rm = T)) %>%
  pivot_wider(id_cols = c(stationname, jaar), names_from = parametername, values_from = median) %>%
  mutate(`Getijslag in cm` = `Hoogwater in cm+NAP` - `Laagwater in cm+NAP`) %>%
  select(stationname, jaar,`Hoogwater in cm+NAP`,`Laagwater in cm+NAP`,`Getijslag in cm`) %>%
