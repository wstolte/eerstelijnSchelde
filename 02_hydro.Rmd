...
trendstations <- c("Europlatform","Vlakte van de Raan","Cadzand","Westkapelle","Breskens Veerhaven","Vlissingen","Gat van Borssele","Overloop van Hansweert",
                   "Terneuzen","Hansweert","Walsoorden","Baalhoek","Schaar van de Noord","Bath")
...
df2 <- df %>%
  ...
  group_by(stationname, jaar, parametername, latitude, longitude) %>% summarise(median = median(value, na.rm = T)) %>%
...
plotTrendsByLocation(df2, "Getijslag in cm")
```
...
plotTrendsByLocationClass(df, "Amplitude in cm", "COMP 65 M2")
```
...
plotTrendsByLocationClass(df, "Amplitude in cm", "COMP 113 M4")
```
...
plotTrendsByLocationClass(df, "Amplitude in cm", "COMP 77 S2")
```
...
plotTrendsByLocationClass(df, "Fase (g) in graden tov MET", "COMP 65 M2")
```
...
plotTrendsByLocationClass(df, "Fase (g) in graden tov MET", "COMP 77 S2")
```
